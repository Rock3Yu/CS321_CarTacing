...
class TestPolicy(Policy):
    def action(self,observation,reward): #receive observation,reward,done,info
        ...
        y=1
        num_cri=0
        cri_vel=observation[1][num_cri]#adversary is front of the agents
        
        cri_pos=observation[2][num_cri]
        
        max1=max(abs(cri_pos[0]),abs(cri_pos[1]))
        #right left up down
        return np.array([0.,max(cri_pos[0]/max1,0.),max(-cri_pos[0]/max1,0.),max(cri_pos[1]/max1,0.),max(-cri_pos[1]/max1,0.)]) 
